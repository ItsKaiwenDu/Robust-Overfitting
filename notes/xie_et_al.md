...
- **Heterogeneous attacks:** Attacks with different structure, such as fine-grained Lp noise vs. global color changes or spatial transformations.
...
2. **Dispatch:** Use predicted threat domain to apply a learned frequency mask and route sample through a threat-specific expert using FC-Conv layers.
...
- The authors also evaluate adaptive attacks aimed at threat-domain classifier, an important safeguard because conditional defenses can otherwise appear robust only when attacker ignores their routing logic.

## Why This Matters for Our Project

TaFD is closest of three new papers to our frequency-domain motivation. It supports idea that attack types can have meaningful and separable spectral structure, and it gives a plausible reason why mixing pixel-space and low-frequency attacks might change training trajectory.

However, TaFD is not our planned implementation. It changes model architecture, learns frequency masks, clusters attack prototypes, predicts attack domains, and routes examples to specialized experts. Our controlled study should keep PreActResNet-18 architecture fixed and change only training attack domain.
