...
* **Deliverables:**
  ...
  * [`po_eval.pdf`](report/pixel-only/baseline/po_eval.pdf): Chart showing robust overfitting with peak at Epoch 105.
  * [`po_train.pdf`](report/pixel-only/baseline/po_train.pdf): Detailed training and evaluation accuracy/loss curves.
...
## Week 11 (Completed)
* **Progress Report:** This week, I double-checked our last week’s low-frequency evaluation results, and confirmed that our setup is indeed correct, from training and evaluation scripts to our plotting script. I completed remaining two experiments on Lambda Labs: pixel-only and mixed-domain condition. Both use same seeds as low-frequency, which is 42 through 46, same setup, same training and evaluation protocol as before. Since we ran these experiments in parallel, our total time spent close to 10 hours, and our total cost is $115.38. Checkpoints are downloaded as well as uploaded to HuggingFace platform as well so anyone can see and download as well; evaluation results are also saved in our reports folder. I also double-verified pixel-only and mixed-domain results against training and evaluation implementations, including DCT-masked attack configuration, checkpoints, and all four reported metrics. So at this point, low-frequency-only, pixel-only, and mixed-domain experiments are all complete. Now what we are left with is writing our final research report and updating our presentation slides, which will be my main objective for next week. I actually gave myself a head start in writing the report, such as having a general skeleton structure ready, and I’m currently in the progress of writing out our results section. Of course, there will be a final review at the end to ensure everything is correct, from statistics and logics to grammars and typos.
...
## Week 12 (Completed)
* **Progress Report:** This week, I completed the final research report and presentation slides. The final report documents the motivation, related work, methodology, evaluation protocol, five-seed results across the pixel-only, low-frequency-only, and mixed-domain conditions, discussion, limitations, and conclusion. I also listened to your suggestions last time, including “separate oscillating mixed-domain results,” “state main negative result,” and “mentioned your name inside acknowledgment section” (both in report and in presentation slide). The presentation is also relatively complete, but I’ll still need to polish a little bit after our meeting, but it now includes background information on adversarial training and robust overfitting, our research question, hypothesis, experimental setup, main findings, and contribution, with speaker notes for verbal delivery. I previously sent my final research report draft to you via email, and if everything checks out, then we should be ready to go.
