...
## Week 10 (Completed)
* **Progress Report:** This week, I ran 5 full 200-epoch low-frequency-only trainings and evaluations on Lambda Labs, using seeds 42 through 46, downloaded and saved 40 checkpoints across each seed (so 200 checkpoints in total) and TensorBoard logs for each run, which are also visualized as training-curve charts in the [`overall/`](report/low-frequency-only/overall/) and [`seed-*`](report/low-frequency-only/) folders. Next, I evaluated every checkpoint on clean images, pixel-space PGD-20, low-frequency DCT-masked PGD-20, and their union robustness (both domains). Evaluation results are saved in [`report/low-frequency-only/`](report/low-frequency-only/): [`overall/`](report/low-frequency-only/overall/) contains five-seed mean $\pm$ 1 SD evaluation and training curves plus a summary CSV, while [`seed-42/`](report/low-frequency-only/seed-42/) through [`seed-46/`](report/low-frequency-only/seed-46/) each contain that seed's evaluation CSV and evaluation and training charts. Total cost for us to train and evaluate on Lambda Labs is \$56.62 USD, a total of 8.40 hours spent, and no unexpected behaviors observed along the way. Finally, based on the training and evaluation charts for the low-frequency-only domain, we see that: mean low-frequency robustness increased from 61.80\% to 84.50\% ($\pm$ 0.56 percentage points) at epoch 200, with pixel-space and union robustness remaining essentially 0\% throughout. This matches our prediction that training against the restricted, smooth low-frequency perturbations would produce strong low-frequency robustness but would not transfer to the pixel space domain, which was the reason for the union robustness only fluctuating around 0\%.
...
## Week 11 (Completed)
* **Progress Report:** This week, I completed the remaining two five-seed, 200-epoch experiments on Lambda Labs: pixel-only training and the epoch-wise mixed-domain condition. Both use seeds 42--46, the same PreActResNet-18/CIFAR-10 setup, perturbation budget, optimizer, and checkpoint schedule as the completed low-frequency-only condition. Each condition produced 40 checkpoints per seed (200 checkpoints per condition), and I evaluated every checkpoint using clean images, pixel-space PGD-20, low-frequency DCT-masked PGD-20, and per-image union robustness. I double-checked the low-frequency, pixel-only, and mixed-domain outputs against the training and evaluation implementations, including the DCT-masked attack configuration, checkpoint metadata, and all four reported metrics. The union metric was verified as a worst-case metric: an image counts as robust only when it is classified correctly under both attacks, rather than by combining the two aggregate accuracy values. Mixed-domain checkpoints also retain the selected attack domain and full seeded epoch schedule, which makes the attack sequence reproducible.

  The new pixel-only control reproduced the robust-overfitting behavior from the original baseline consistently across all five seeds. Mean pixel-PGD-20 and union robustness both peaked at **51.22%** at epoch **105**, then fell to **42.66%** by epoch 200, a decline of **8.56 percentage points**. Every seed reached its pixel-robust peak at epoch 105; clean accuracy instead peaked later at **85.07%** at epoch 155. The separate Rice et al. reproduction baseline showed the same pattern (pixel-PGD-20 peak **45.91%** at epoch 105 and final value **36.18%**, a **9.73-point** decline), so it remains a useful independent reference but was correctly excluded from the five-seed aggregate.

  The completed low-frequency-only condition provides a sharply different comparison. Its matched low-frequency-PGD-20 robustness peaked at **92.74%** at epoch 195 and ended at **92.70%**, only **0.05 points** lower, while pixel-PGD and union robustness remained **0.00%** throughout. Thus, low-frequency-only training was highly stable in its own threat domain but did not transfer to pixel-space attacks. In contrast, the epoch-wise mixed-domain condition did not converge to stable simultaneous robustness. Its aggregate union robustness peaked at **40.79%** at epoch 85 and ended at **19.33%**, but this apparent **21.46-point** decline is substantially confounded by the schedule: across all seed-checkpoint observations, mean pixel robustness was **42.36%** after a pixel-training epoch versus **3.53%** after a low-frequency-training epoch (point-biserial correlation **r = 0.971**). Clean and low-frequency performance show the complementary recency effect. Therefore, the mixed-domain result should not be interpreted as evidence of stronger robust overfitting; it instead shows that alternating one attack domain for an entire epoch produces domain-specialized model states. A future simultaneous-robustness test should mix attacks within batches or use a combined inner maximization.

  I also started the final research report ahead of the Week 12 schedule in [`report/main.tex`](report/main.tex). Its basic structure is in place and the results section is complete, including the three-condition comparison; next week I will finish the remaining report work and develop the presentation slides.

* **Deliverables:**
  * [`report/pixel-only/overall/po_overall.md`](report/pixel-only/overall/po_overall.md): Five-seed pixel-only summary with peak epochs, accuracy declines, and interpretation.
  * [`report/pixel-only/seed-42/`](report/pixel-only/seed-42/) through [`report/pixel-only/seed-46/`](report/pixel-only/): Per-seed evaluation CSVs, result summaries, and training/evaluation plots.
  * [`report/mixed-domain/overall/mdo_overall.md`](report/mixed-domain/overall/mdo_overall.md): Five-seed mixed-domain summary, including the schedule-recency analysis.
  * [`report/mixed-domain/seed-42/`](report/mixed-domain/seed-42/) through [`report/mixed-domain/seed-46/`](report/mixed-domain/): Per-seed evaluation CSVs, summaries, and training/evaluation plots with recorded attack schedules.
