...
**Purpose:** Define experiment before implementation so that three
...
once per epoch affect timing and severity of robust overfitting, relative
to corresponding single-domain training conditions?

For every training condition and evaluation threat, report:

- **peak epoch:** saved-checkpoint epoch with highest robust accuracy;
- **peak robust accuracy:** accuracy at that checkpoint; and
- **post-peak decline:** peak robust accuracy minus robust accuracy at epoch
  200.

Ties are resolved by selecting earliest checkpoint. Because checkpoints
are saved every five epochs, peak epoch is measured to that resolution.
...
or select threats at example or PGD-step level, or change model
architecture. Here, chosen attack is fixed for every batch of one epoch.
...
| Model | PreActResNet-18 with existing CIFAR-10 normalizer |
| Input domain | Raw, unnormalized RGB tensors in `[0, 1]`; normalization is applied only immediately before model |
...
The already-completed pixel-only run remains historical baseline. It will
be re-evaluated under new paired evaluation protocol; new low-frequency
and mixed-domain runs use exact settings above.

## Training conditions

1. **Pixel-only:** Use existing pixel-space PGD-10 attack in every epoch.
2. **Low-frequency-only:** Use low-frequency DCT-masked PGD-10 in every
   epoch.
3. **Mixed-domain:** At beginning of each epoch, select one attack domain
...
For each image and RGB channel independently, use orthonormal two-
...
For CIFAR-10's 32x32 images, fixed mask is

\[
M_{u,v}=1 \quad \text{when } 0 \leq u < 8 \text{ and } 0 \leq v < 8,
\]

and zero otherwise. Thus, each channel retains top-left 8x8 DCT
...
stored in run configuration and included in code as a deterministic
...
low-pass-projected with `P_M`. At every PGD step, compute image-space loss
...
`Pi_{infinity, X}` first clips perturbation coordinatewise to
`[-epsilon, epsilon]`, then clips `X + delta` to `[0, 1]` and subtracts `X`.
This keeps *final evaluated image* valid and guarantees
`max(abs(X_adv - X)) <= 8/255`. Pixel PGD retains existing random-start,
...
value is clipped at 0 or 1, resulting final perturbation can contain a
...
For every low-frequency attack invocation, implementation will record
the batch mean and maximum *out-of-mask energy fraction* of final
perturbation,
...
clipping creates material leakage (mean above 1%), result will be reported
as a limitation and later strict-subspace alternative will require a
coefficient-space constrained optimizer; it will not be silently described as
an exact constrained attack.

## Checkpoint evaluation

Every saved checkpoint is evaluated over complete 10,000-image CIFAR-10
test set, using following fixed suite:

| Metric | Attack |
| --- | --- |
| `clean_acc`, `clean_loss` | No attack |
| `pixel_pgd20_acc`, `pixel_pgd20_loss` | Pixel-space PGD-20, `epsilon=8/255`, `alpha=2/255`, random start |
| `low_frequency_pgd20_acc`, `low_frequency_pgd20_loss` | DCT-masked PGD-20 with fixed 8x8 mask and same image-space budget/step size |
| `union_acc` | Per-example intersection of two attack-correct indicators: correct only when both attacks leave example correctly classified |

The union metric is computed per example, not as minimum of two
aggregate accuracies. Its loss is intentionally not reported: union
...
- `evaluation_results.csv` with all four metrics in evaluation table;
- plots of clean, pixel-PGD, low-frequency-PGD, and union accuracy versus
  epoch; and
- exact launch command in `command.txt`.

No results from a failed, restarted, or diagnostically different run may be
merged into primary run directory. Such runs receive their own directory
and a clear suffix.
...
1. Pixel and low-frequency attacks return finite tensors on selected
...
runs. If time permits after primary conditions, rerun all three conditions
with seeds 43 and 44 and report mean and range without changing any other
setting.

The frequency mask follows transform-mask-inverse-transform approach used
to restrict low-frequency adversarial searches by Guo, Frank, and Weinberger.
The per-example union metric follows multi-perturbation evaluation
...
- Maini, P., Wong, E., & Kolter, J. Z. (2020). *Adversarial Robustness Against
  Union of Multiple Perturbation Models.* ICML 2020.
