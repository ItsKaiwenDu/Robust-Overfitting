% ICML 2026 version of the Robust-Overfitting final report.
% Compile from report/icml; figures are loaded from the report directories.
% Keep the official icml2026.sty file in this directory.
...
% Improves compatibility between hyperref and the algorithm package loaded
% by the official ICML style.
\newcommand{\theHalgorithm}{\arabic{algorithm}}

% Preprint mode shows author information without claiming ICML acceptance.
% For anonymous ICML review, remove [preprint]. Use [accepted] only after acceptance.
...
% Keep multiple double-column floats packed at the top of float-only pages.
\makeatletter
...
\icmltitlerunning{Robust Overfitting Across Pixel-Space and Low-Frequency Adversarial Training}

\begin{document}

\twocolumn[
  \icmltitle{Robust Overfitting Across Pixel-Space and Low-Frequency Adversarial Training}

  \begin{icmlauthorlist}
    \icmlauthor{Kaiwen Du}{scu}
  \end{icmlauthorlist}

  \icmlaffiliation{scu}{Department of Mathematics and Computer Science, Santa Clara University, Santa Clara, California, USA}
  \icmlcorrespondingauthor{Kaiwen Du}{TheKaiwenDu@gmail.com}

  \icmlkeywords{adversarial training, robust overfitting, projected gradient descent, low-frequency perturbations}
...
\citep{goodfellow2015,rice2020}. Adversarial training addresses this threat by
...
then decline even as robust training loss continues to decrease.
\citet{rice2020} describe this phenomenon as robust overfitting;
\citet{yu2022} further investigate its causes. This gap between the best and
...
models \citep{tramer2019,guo2020,maini2020,bu2023}. We therefore examine how the training attack affects when robust test accuracy
...
attack \citep{goodfellow2015,madry2018}. In adversarial training, the model is
...
\citet{rice2020} documented robust overfitting across data sets and
...
an early checkpoint using a held-out validation set. \citet{yu2022}
...
smooth, broad image changes \citep{guo2020}. Frequency-aware work suggests that
frequency components can make different contributions to prediction and robust
learning \citep{chen2022,bu2023}. Meanwhile, robustness against multiple
perturbation models can be imbalanced: a model trained for one threat may
remain weak for another \citep{tramer2019,maini2020}. Recent work also relates
heterogeneous threat models to distinct spectral signatures and possible
negative transfer \citep{xie2026}.
...
only on the example with the highest loss \citep{tramer2019}. Maini
...
direction at every attack step \citep{maini2020}. We instead use a
...
\begin{equation}
  \begin{aligned}
    \delta_{t+1}
    &=\Pi_{\mathcal{S}_x}\left(\delta_t+\alpha\,
      \mathrm{sign}\right. \\
    &\quad\left.\left(\nabla_\delta
...
\citet{he2016}, with the implementation provided by \citet{rice2020}. Models
are trained on CIFAR-10
\citep{krizhevsky2009}. Training uses
...
Following the description of robust overfitting by \citet{rice2020},
...
\bibitem[Rice et al.(2020)]{rice2020}
...
\bibitem[Tram\`er and Boneh(2019)]{tramer2019}
...
\bibitem[Maini et al.(2020)]{maini2020}
