...
Project records identify NVIDIA A10 (24~GB PCIe) Lambda Labs instances as the
full-training platform. The retained TensorBoard logs for all 15 full runs
...
measure. The source code seeds Python, NumPy, and PyTorch for every run, seeds
all CUDA devices when available, and sets \texttt{cudnn.deterministic=True}
and \texttt{cudnn.benchmark=False}. It does not enable PyTorch's global
...
The hypothesis is supported in its deliberately narrow, descriptive form:
changing the training domain changed the observed robust-accuracy trajectories.
Pixel-only training produced a 51.22\% aggregate pixel-robustness peak at
epoch 105 followed by an 8.56-point decline, whereas low-frequency-only
training's matched low-frequency accuracy declined only 0.05 points from its
peak. Mixed-domain training was distinct from both baselines, but not because
it established stable coverage across attacks: pixel robustness averaged
42.36\% after pixel-training epochs and 3.53\% after low-frequency-training
epochs, with a point-biserial correlation of $r=0.971$ between the preceding
domain and pixel robustness. Thus the results support evaluation-domain
dependence and schedule sensitivity, but do not support a claim that epoch-wise
mixing mitigates robust overfitting or reveals its causal mechanism.
...
such as the lower of the two per-attack validation accuracies, rather than a
post-hoc choice from test curves. The test set should then be evaluated once
...
\begin{itemize}
  ...
  \item \textbf{Execution provenance.} Checkpoint metadata verifies each
  condition, seed, cutoff, and mixed-domain schedule, and TensorBoard logs
  verify the recorded training traces. However, immutable command transcripts
  and an exact cloud software/driver manifest were not retained. The reported
  configuration can be rerun from the source, but bitwise reproduction across
  machines and library versions is not established.
\end{itemize}
...
conjunction robustness. Once-per-epoch mixed-domain training did not yield
stable simultaneous robustness; its pixel robustness was strongly determined
by the most recent training domain. These findings are limited to
PreActResNet-18, CIFAR-10, the selected DCT cutoff, and the specified
finite-step attacks; the most useful next experiment is a validation-selected,
batch- or example-level multi-threat training objective evaluated under a
broader attack suite.
...
\end{itemize}
Code, committed evaluation results, report-check artifacts, and current
