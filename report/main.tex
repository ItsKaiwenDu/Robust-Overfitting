...
\leq\epsilon\}$, where $\epsilon=8/255$. The pixel-space threat set is
$\mathcal{S}_x=\{\delta\in\mathcal{B}_\infty(\epsilon):x+\delta\in[0,1]^{3\times32\times32}\}$.
...
and $\linf$ budget. They differ in the attack used to generate training
examples. The low-frequency attack changes both the search domain and the
...
\Cref{app:schedules} reports the realized domain counts and the domains at
the aggregate pixel peak and final checkpoint.
...
We used NVIDIA A10 (24~GB PCIe) instances on Lambda Labs for all full
training runs. The retained TensorBoard logs for all 15 full runs
...
\Cref{app:checks} compares single-start PGD-20 with three-restart PGD-50
on two pixel-only checkpoints using a fixed 256-image subset. This diagnostic
does not assess the strength of the primary low-frequency attack.

\paragraph{Metric interpretation.}
Conjunction accuracy measures per-example correctness under both generated
attacks; it does not require a correct clean prediction. The CSVs and plot
legends call this quantity ``union robustness.'' It is an empirical two-attack
...
\end{align}
where ROG denotes the robust-overfitting gap in percentage points. Ties are
resolved by choosing the earliest evaluated epoch. Aggregate peaks and ROGs
in \cref{tab:main-results} are calculated
from the condition mean trajectory using unrounded values, whereas
\cref{app:per-seed} reports each seed's own earliest peak. For mixed-domain
...
adjust for epoch or training history. Individual seeds reach their conjunction
...
Pixel-only training achieves the highest final pixel and conjunction accuracy,
...
The results support the hypothesis descriptively: the training attack changes
...
These experiments do not isolate a causal mechanism. Frequency bias,
optimization dynamics, and negative transfer are possible explanations,
but distinguishing them requires further controls. In particular, the
...
such as per-example conjunction accuracy under the chosen attacks. The
...
\begin{itemize}
  ...
  inner maximization over their union or an exact worst-case robustness result.
...
with the most recent training domain. These findings are limited to
...
\begin{table}[htbp]
\centering
\caption{Per-seed pixel-PGD-20 results for pixel-only training. Accuracy is in percent;
gaps are in percentage points.}
...
training. Accuracy is in percent; gaps are in percentage points.}
...
is in percent; gaps are in percentage points. These raw gaps
