...
is the fraction of test images correctly classified under both attacks. Under pixel-only training, mean pixel robustness peaked at 51.22\%
...
test accuracy to its accuracy at epoch 200; \cref{sec:analysis-plan} gives the
formal definition.
...
\begin{enumerate}
  ...
  \item joint accuracy, where an image counts as correct only if it is
...
Condition & Clean & Pixel & Low freq. & Joint & Peak (epoch) & ROG & Peak (epoch) & ROG \\
\midrule
...
remains 84.40\%. Pixel and joint accuracy are effectively identical
...
robust overfitting alone caused the raw 21.46-point joint gap.
...
\begin{itemize}
  ...
  \item \textbf{Joint accuracy versus union attack.} The joint metric is
...
joint accuracy. Once-per-epoch mixed-domain training did not yield
