...
Li et al. study adversarial training through an image's Fourier **amplitude** and **phase**. They observe that attacks appear to damage phase patterns more than amplitude patterns, and propose **Dual Adversarial Training (DAT)** to make the model rely less on amplitude and more on phase.

DAT creates a new version of each training image by keeping its phase but mixing its amplitude with an adversarially generated amplitude. The model is adversarially trained on both the original and recombined images.

## Key Terms

- **Amplitude:** the size or strength of each Fourier component. It is often associated with image appearance or style.
- **Phase:** the alignment of Fourier components. It often carries more of an image's spatial structure and object information.
- **Recombined image:** an image reconstructed with the original image's phase and a mixed amplitude.
- **Adversarial Amplitude Generator (AAG):** a small generator trained to produce an amplitude that makes learning harder, so the classifier cannot depend too much on amplitude.
...
The authors form hybrid test images: one has the adversarial image's phase and the clean image's amplitude; the other has the adversarial amplitude and clean phase. Their standard CIFAR-10 model performs worse when the adversarial **phase** is used. They take this as evidence that attacks affect phase-related information more strongly.
...
2. DAT mixes that amplitude with the image's own amplitude and keeps the original phase.
3. It trains on PGD-style adversarial examples from both the original and recombined images, with a consistency loss that encourages both versions to receive the same prediction.
4. It uses split batch normalization and a modified adversarial-example loss to reduce the number of attack steps needed during training.

The authors also give a conditional theoretical result: under their augmentation assumption and a linear softmax classifier on top of learned features, the weights on amplitude-derived features shrink. This supports their proposed mechanism, but it is not a universal proof that all robust models should ignore amplitude.
...
**Takeaway:** Li et al. show that frequency-domain structure can help robustness, but our experiment isolates a cleaner question: which *allowed perturbation frequency band* changes robust overfitting, when everything else stays the same?
